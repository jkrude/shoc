...
volatile ALPAKA_FN_ACC int g_mutex=0;
//Store the last used value of g_mutex2
volatile ALPAKA_FN_ACC int g_mutex2=0;
...
// ****************************************************************************
template<typename TAcc>
ALPAKA_FN_ACC void __gpu_sync(TAcc const& acc, int blocks_to_synch)
{
    __syncthreads();
...
volatile ALPAKA_FN_ACC int g_q_offsets[1]={0};
//store the frontier length for the next iteration
volatile ALPAKA_FN_ACC int g_q_size[1]={0};
...
struct BFS_kernel_one_block_spill {
    ...
    void operator()(
        ...
        const unsigned int max_local_mem) const{

        volatile sharedMemExtern( s_mem, unsigned int );
        ...
        volatile sharedMem( b_q_length, cupla::Array<unsigned int, 1> );
        volatile sharedMem( b_offset, cupla::Array<unsigned int, 1> );
...
struct BFS_kernel_SM_block_spill {
   ...
   ALPAKA_FN_ACC
    void operator()(
        ...
        volatile sharedMemExtern( b_q, unsigned int );
        volatile sharedMem( b_q_length, cupla::Array<unsigned int, 1> );
        volatile sharedMem( b_offset, cupla::Array<unsigned int, 1> );
